...
Avant tout, il est important de rappeler que les règles des objects calisthéniques ne sont qu'un **guide pour avoir un code plus propre**, il n'est en aucun cas *obligatoire* de respecter ces règles, bien que *conseillé*.
...
class Board {
    ...
    private void collectRow(StringBuilder buffer, int row) {
        for (int i = 0; i < 10; i++) {
...
class Board {
    public String boardRepresentation() {
        ...
        for (Location loc : squares()) {
            buffer.append(loc.current.representation.substring(0, 1));
        }
...
class Location {
    ...
    public void addTo(StringBuilder buffer) {
        current.addTo(buffer);
    }
}
```
